...
class EhealthItem extends Component {
    ...
    onCheckEmty = (object, type) => {
        ...
        if (typeof object === 'object') {
            for (let key in object) {
                if (object[key]) {
                    return (
                        !type ? this.renderItemCheckupContract(this.props.internalMedicine) : this.renderResultTes(this.props.itemMedical)
                    )
                }
            }
        }
        ...
        if (item.result || item.result === true) {
            return (
                <View style={styles.content} key={index}>
                    ...
                    <Text style={styles.value}>{item.result === true ? 'Có' : item.result === false ? 'Không' : item.result}</Text>
                </View>
            )
        }
    }
    renderResultTes(item) {
        ...
        let anamnesis = item?.anamnesis?.filter(obj => obj.result !== false && obj.result) // tiền sử
        let description = item?.description.filter(obj => obj.result !== false && obj.result) // Thông tin khám
        ...
        return (
            <View style={{ flex: 1, }} >
                <ScrollView
                    ...
                    style={{ padding: 10, }} >
                    <View style={styles.viewServiceName}>
                        <Text style={styles.txServiceName}>{'DỊCH VỤ KHÁM BỆNH THEO YÊU CẦU'}</Text>
                        {/* <View style={styles.viewBorder}></View> */}
                    </View>
                    ...
                    {
                        description && description.length ? <View>
                            <View style={styles.viewServiceName}>
                    ...
                    {
                        anamnesis && anamnesis.length ? <View>
                            <View style={styles.viewServiceName}>
                    ...
                    {conclusion ? <View>
                        ...
                        <View style={styles.conclusion} >
                            {conclusion.diagnostic?.initial ? <View style={{ flex: 1 }}><Text style={styles.labelConclusion}>{'Chẩn đoán sơ bộ'}</Text>
                                <View style={styles.viewRow}>
                                    <ScaleImage source={require("@images/new/ehealth/ic_dot.png")} width={5} style={{ marginRight: 10, tintColor: '#0291E1' }} />
                                    <View style={{ width: '100%', }}><Text style={styles.valueConclusion}>{conclusion.diagnostic?.initial}</Text></View>
                                </View></View> : null}
                            {conclusion.diagnostic?.detail ? <View style={{ flex: 1 }}><Text style={styles.labelConclusion}>{'Chẩn đoán chi tiết'}</Text>
                                <View style={styles.viewRow}>
                                    <ScaleImage source={require("@images/new/ehealth/ic_dot.png")} width={5} style={{ marginRight: 10, tintColor: '#0291E1' }} />
                                    <View style={{ width: '100%', }}><Text style={styles.valueConclusion}>{conclusion.diagnostic?.detail}</Text></View>
                                </View></View> : null}
                            {conclusion.diagnostic?.diseases ? <View style={{ flex: 1 }}><Text style={styles.labelConclusion}>{'Chẩn đoán bệnh'}</Text>
                                <View style={styles.viewRow}>
                                    <ScaleImage source={require("@images/new/ehealth/ic_dot.png")} width={5} style={{ marginRight: 10, tintColor: '#0291E1' }} />
                                    <View style={{ width: '100%', }}><Text style={styles.valueConclusion}>{conclusion.diagnostic?.diseases}</Text></View>
                                </View></View> : null}
                            {conclusion.diagnostic?.other ? <View style={{ flex: 1 }}><Text style={styles.labelConclusion}>{'Chẩn đoán khác'}</Text>
                                <View style={styles.viewRow}>
                                    <ScaleImage source={require("@images/new/ehealth/ic_dot.png")} width={5} style={{ marginRight: 10, tintColor: '#0291E1' }} />
                                    <View style={{ width: '100%', }}><Text style={styles.valueConclusion}>{conclusion.diagnostic?.other}</Text></View>
                                </View></View> : null}
                            {conclusion.advice ? <View style={{ flex: 1 }}>
                                <Text style={styles.labelConclusion}>{'Lời dặn'}</Text>
                                <View style={styles.viewRow}>
                                    <ScaleImage source={require("@images/new/ehealth/ic_dot.png")} width={5} style={{ marginRight: 10, tintColor: '#0291E1' }} />
                                    <View style={{ width: '100%', }}><Text style={styles.valueConclusion}>{conclusion.advice}</Text></View>
                                </View>
                            </View> : null}
                            {conclusion.note ? <View style={{ flex: 1 }}><Text style={styles.labelConclusion}>{'Ghi chú'}</Text>
                                <View style={styles.viewRow}>
                                    <ScaleImage source={require("@images/new/ehealth/ic_dot.png")} width={5} style={{ marginRight: 10, tintColor: '#0291E1' }} />
                                    <View style={{ width: '100%', }}><Text style={styles.valueConclusion}>{conclusion.note}</Text></View>
                                </View></View> : null}
    ...
    render() {
        ...
        console.log('itemMedical: ', itemMedical);
...
const styles = StyleSheet.create({
    ...
    content: { flexDirection: 'row', justifyContent: 'space-between', marginVertical: 2, },
    labelConclusion: { fontSize: 14, fontWeight: 'bold', color: '#000000', marginBottom: 5, textAlign: 'left' },
    valueConclusion: { fontSize: 14, color: '#000000', fontFamily: 'Muli', textAlign: 'left', width: '90%' },
    ...
    viewBorder: { height: 1, backgroundColor: '#00000020', width: '40%', alignSelf: 'flex-end', marginRight: -10, },
    ...
    txServiceName: { alignSelf: 'flex-start', fontWeight: 'bold', fontSize: 16, color: '#0291E1', flex: 1 },
    viewItem: {
    ...
    viewServiceName: { flexDirection: 'row', justifyContent: 'space-around', marginVertical: 10, },
    ...
    value: { fontSize: 14, color: '#000000', fontFamily: 'Muli', marginBottom: 10, alignItems: 'flex-end', width: '50%', textAlign: 'right' },
    label: { fontSize: 14, fontWeight: 'bold', color: '#000000', marginBottom: 5, width: '50%' },
