...
class MedicalTestResult extends Component {
  ...
  onGetData = result1 => {
    ...
    if (result1) {
      if (
        result1?.ListResulHuyetHoc?.length &&
        result1?.ListResulHuyetHoc[0]?.ListMedical?.length
      ) {
        hasResult = true;
      }
      if (
        result1?.ListResulHoaSinh?.length &&
        result1?.ListResulHoaSinh[0]?.ListMedical?.length
      ) {
        hasResult = true;
      }
      if (
        result1?.ListResulOther?.length &&
        result1?.ListResulOther[0]?.ListMedical?.length
      ) {
        hasResult = true;
      }
      if (
        result1?.ListResulViSinh?.length &&
        result1?.ListResulViSinh[0]?.ListMedical?.length
      ) {
        hasResult = true;
      }
    }
    if (!hasResult) {
      this.setState({
        hasResult: false,
        medicalTestResult: result1?.ListResulHoaSinh || [],
      });
      return null;
    }
  ...
  renderMedical(item, index) {
    let borderBottomWidth =
      index == this.state.currentGroup.value.ListMedical.length - 1 ? 0.6 : 0;
    if (
  ...
  render() {
    if (!this.state.currentGroup || !this.state.hasResult) {
      if (
        this.state?.medicalTestResult?.length &&
        (this.state?.medicalTestResult[0]?.SummaryResult ||
          this.state?.medicalTestResult[0]?.Image?.length != 0)
      ) {
    ...
    const tableHead = ['TÊN XÉT NGHIỆM', 'KẾT QUẢ'];
        ...
        <ActionSheet
          ...
          onPress={index => {
            if (index <= this.state.medicalTestResult.length - 1) {
              this.setState({
                currentGroup: this.state.medicalTestResult[index],
              });
            }
          }}
        />
