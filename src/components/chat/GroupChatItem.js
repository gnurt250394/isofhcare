...
class GroupChatItem extends React.Component {
  ...
  showData(props) {
    ...
    dataCacheProvider.read(this.state.userId, 'group_chat_' + item.id, s => {
      ...
      firebaseUtils
        .getGroupName(this.state.userId, groupId)
        .then(x => {
          let info = {
            name: x.name ? x.name : 'Tin nhắn',
            avatar: x.avatar ? x.avatar.absoluteUrl() : '',
          };
          this.setState(info);
        ...
        this.snapshot = message
          ...
          .onSnapshot(snap => {
            snap.docChanges().forEach(item => {
              ...
              firebaseUtils
                .getUnReadMessageCount(this.state.userId, props.group.id)
                .then(x => {
                  this.setState({
                    unReadCount: x,
                  });
  ...
  highlighMessage(item) {
    try {
      return item.readed.indexOf(this.state.userId + '') == -1
        ? '900'
        : 'normal';
    } catch (error) {}
    return 'normal';
            ...
            <View style={{flexDirection: 'row'}}>
              <View
                ...
                <Text style={{fontWeight: 'bold', fontSize: 15, flex: 1}}>
                  {item.name ? item.name : this.state.name}
                </Text>
            ...
            {this.state.lastMessage && (
              <View>
                <Text
                  ...
                  style={{
                    marginTop: 5,
                    fontWeight: this.highlighMessage(this.state.lastMessage),
                  }}>
                  {this.state.lastMessage.type == 4
                    ? '[Hình ảnh]'
                ...
                {this.state.lastMessage.fromCache && (
                  <Text
                    ...
                    {this.state.lastMessage.createdDate
                      .toDateObject()
                      .getPostTime()}
                  </Text>
                )}
