...
const HistoryCumulativeScreen = () => {
    ...
    const [state, _setState] = useState({ data: [], page: 0, size: 20, isLoading: true, list: [], refreshing: true })
    ...
    const Item = ({ item }) => {
        const icon = item.point > 0 ? require("@images/new/account/ic_out.png") : require("@images/new/account/ic_in.png")
        return (
    ...
    const listEmpty = () => !state.isLoading && <Text style={styles.txtEmpty}>Không có dữ liệu</Text>
        ...
        >
            <View style={styles.containerCount}>
                ...
                <View style={styles.groupCount}>
                    <Text style={styles.txtCount}>{userApp?.currentUser?.accumulatedPoint}</Text>
                    <ScaledImage
            ...
            <Text style={styles.txtHelp}>Mỗi tài khoản đăng kí được bạn giới thiệu sẽ tích lũy được 10 điểm.</Text>
            <SectionList
                sections={state.data}
