...
class ListBookingHistoryScreen extends Component {
  ...
  getListProfile = () => {
    profileProvider.getListProfile().then(s => {
      if (s?.length) {
        let phoneProfile = s.reduce(
          (total, item) =>
            total + item?.profileInfo?.personal?.mobileNumber + ',',
          '',
        );
  ...
  getData = () => {
    ...
    BookingDoctorProvider.getListBooking(
      phoneProfile,
  ...
  renderBookingOnline = item => {
    return (
      <Text style={[styles.statusTx, styles.flexStart, styles.colorWhite]}>
        Lịch gọi tư vấn
      </Text>
    );
  };
        ...
        <View style={styles.row}>
          <View
            style={[
              ...
              (item.status == 'ACCEPTED' || item.status == 'CHECKIN')
            ...
            <Text style={{marginTop: 10}}>{item.time ? item.time : ''}</Text>
          </View>
          <View style={styles.containerBody}>
            <Text style={styles.txtServiceType}>
            ...
            <View style={{marginVertical: 10}}>
              ...
              <Text style={styles.txtUserName}>
                {item.hospital && item.hospital.name ? item.hospital.name : ''}
              </Text>
            </View>
            <View
              style={{
  ...
  renderStatus = item => {
    if (
      item.invoice.payment == constants.PAYMENT_METHOD.NONE ||
      ...
      return (
        <Text style={[styles.statusReject, styles.flexStart, styles.colorRed]}>
          Chưa thanh toán
        </Text>
      );
    } else {
      switch (item.status) {
        case 'NEW':
          return (
            <Text
              style={[styles.statusTx, styles.flexStart, styles.colorWhite]}>
              Chờ duyệt
            </Text>
          );
        case 'ACCEPTED':
        ...
        case 'CANCELED':
          return (
            <Text
              style={[styles.statusReject, styles.flexStart, styles.colorRed]}>
              Đã hủy
            </Text>
          );
        case 'COMPLETED':
