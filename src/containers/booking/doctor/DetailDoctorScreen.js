...
class DetailsDoctorScreen extends Component {
      ...
      .then(s => {
        if (s) {
          if (s && s.workingProcesses && s.workingProcesses.length) {
            s.workingProcesses.length &&
              s.workingProcesses.sort(function(a, b) {
  ...
  onCallVideo = () => {
    ...
    if (!this.state.profileDoctor.userId) {
      snackbar.show(
        'Bác sĩ hiện tại không online vui lòng đặt lịch gọi khám vào thời gian khác',
      );
      return;
    }
    // if (this.props.userApp.isLogin) {
      ...
      return (
        <View style={styles.flex}>
          {data && data.length > 0
            ? data.map((e, i) => {
                return (
                  <Text style={styles.txtPosition} key={i}>
                    {e.name}
                  </Text>
                );
              })
    ...
    return (
      <View
        style={[
          ...
          {borderBottomWidth: index == dataRate.length - 1 ? 0 : 0.7},
        ]}>
  ...
  renderItemWorking = ({item, index}) => {
    return (
      <View style={styles.viewItemBooking}>
        ...
        <Text style={styles.contentWorking}>{`${
          item.startDate ? item.startDate.toDateObject('-').format('yyyy') : ''
        } - ${
          item.endDate
            ? item.endDate.toDateObject('-').format('yyyy')
            : 'Hiện tại'
        } : ${item.position} tại ${item.workPlace}`}</Text>
      </View>
    );
  };
  renderWorking = () => {
    ...
    if (
      profileDoctor &&
      profileDoctor.workingProcesses &&
      profileDoctor.workingProcesses.length
    ) {
      return (
        <FlatList
  ...
  render() {
    ...
    const {profileDoctor} = this.state;
    const source =
      profileDoctor && profileDoctor.imagePath
          ...
          <View
              ...
              <View style={{flexDirection: 'row', alignItems: 'flex-start'}}>
                ...
                <View style={{paddingLeft: 10, flex: 1}}>
                  ...
                  <Text style={{paddingBottom: 10}}>
                    {profileDoctor?.position?.fullName}
                  </Text>
              ...
              <View style={styles.row}>
                <View style={styles.groupQuantityBooking}>
                  ...
                  <Text style={styles.rating}>
                    {profileDoctor.appointments}
                  </Text>
                </View>
                <View style={styles.groupQuantityBooking}>
                  <Text>{constants.booking.quantity_advisory}</Text>
                  <Text style={styles.rating}>
                    {profileDoctor.onlineAppointments}
                  </Text>
                </View>
                <TouchableOpacity
                  ...
                  <Text style={[styles.rating, {color: '#00CBA7'}]}>
                    {profileDoctor.average ? profileDoctor.average : 0}
                  </Text>
                </TouchableOpacity>
              </View>
            </Card>
            ...
            <Card style={styles.containerInfo}>
              ...
              <View
                ...
                <Text style={styles.colorBold}>
                  {constants.booking.specialist}:
                </Text>
                <View style={{flexDirection: 'row', flexWrap: 'wrap'}}>
                  {profileDoctor.specializations &&
                  profileDoctor.specializations.length > 0
                    ? profileDoctor.specializations.map((e, i) => {
                        return (
                          <Text style={styles.txtPosition} key={i}>
                            {e.name}
                            {i == profileDoctor.specializations.length - 1
                              ? '.'
                              : ', '}
                          </Text>
                        );
          ...
          <View style={styles.containerModal}>
            <View>
              <TouchableOpacity
                ...
                <ScaleImage
                  source={require('@images/new/booking/ic_message.png')}
                  height={50}
                />
            ...
            <View>
              <TouchableOpacity
                ...
                <ScaleImage
                  source={require('@images/new/booking/ic_video_call.png')}
                  height={50}
                />
