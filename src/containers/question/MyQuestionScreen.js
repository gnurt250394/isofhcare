...
class MyQuestionScreen extends Component {
    ...
    onLoad() {
        ...
        questionProvider.search(this.props.userApp.currentUser.id, page, size, (s, e) => {
            this.setState({
                loading: false,
                refreshing: false,
                loadMore: false
    ...
    editPost(item) {
        this.props.navigation.replace("createQuestion", { post: item });
    }
    deletePost(item) {
        this.dialogbox.confirm({
            title: constants.alert,
            content: [constants.msg.question.confirm_delete_post],
            ok: {
                ...
                <View style={{ flexDirection: 'row', margin: 10 }}>
                    ...
                    <View style={{ marginTop: 10, flex: 1 }}>
                        <View style={{ flexDirection: 'row', paddingTop: 5 }}>
                            <Text style={{ fontWeight: 'bold', flex: 1 }} numberOfLines={2} ellipsizeMode="tail">{item.post.title}</Text>
                            <Menu>
                                <MenuTrigger>
                                    <View style={{ padding: 5, paddingLeft: 20, paddingRight: 0 }} onPress={this.deletePost.bind(this, item)}>
                                        <ScaleImage source={require('@images/question/menu.png')} height={20} style={{ marginTop: -5 }} />
                                    </View>
                                </MenuTrigger>
                                <MenuOptions>
                                    <MenuOption onSelect={this.editPost.bind(this, item)} >
                                        <Text>Chỉnh sửa</Text>
                                    </MenuOption>
                                    <MenuOption value={3} disabled={true}>
                                        <View style={{ height: 1, backgroundColor: '#aca' }} />
                                    </MenuOption>
                                    <MenuOption onSelect={this.deletePost.bind(this, item)}  >
                                        <Text style={{ color: 'red' }}>Xoá</Text>
                                    </MenuOption>
                                </MenuOptions>
                            </Menu>

                        </View>                        <View flexDirection='row' style={{ marginTop: 5, flex: 1 }}>
    ...
    render() {
        return (
            <ActivityPanel style={{ flex: 1 }} title="Câu hỏi của tôi" showFullScreen={true}>
                <FlatList
...
export default connect(mapStateToProps)(MyQuestionScreen);