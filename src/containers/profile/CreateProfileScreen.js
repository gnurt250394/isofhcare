...
class CreateProfileScreen extends Component {
    ...
    componentDidMount() {
        ...
        if (dataOld) {
            ...
            this.setState({
                ...
                date: dataProfile && dataProfile.dateOfBirth ? dataProfile.dateOfBirth.toDateObject('-').format('dd/MM/yyyy') : (''),
                txGender: dataProfile && dataProfile.gender == 'MALE' ? 'Nam' : 'Nữ',
                valueGender: dataProfile.gender,
                dobOld: dataProfile && dataProfile.dateOfBirth ? dataProfile.dateOfBirth : '',
                ...
                dob: dataProfile && dataProfile.dateOfBirth ? dataProfile.dateOfBirth.toDateObject('-') : '',
                        ...
                        () => {
                            ...
                            if (age == 0 || age < 14) {
                                var data = {
                                    ...
                                    "nationality": this.state.country ? this.state.country.id : '',
                                    "job": this.state.jobs ? this.state.jobs.id : '',
                                    // "village": village ? village : ' ',
                                    // "passport": this.state.userPassport,
                                    "guardian": {
                                        "fullName": this.state.guardianName ? this.state.guardianName : '',
                                        "mobileNumber": this.state.guardianPhone ? this.state.guardianPhone : '',
                                        "idNumber": this.state.guardianPassport ? this.state.guardianPassport : '',
                                    },
                                    "address": {
                                        "province": province ? province.name.toString() : null,
                                        "city": zone ? zone.id.toString() : null,
                                        "district": district ? district.name.toString() : null,
                                    },
                                }
                            } else if (age >= 14) {
                                var data = {
                                    ...
                                    'fullName': this.state.name,
                                    'email': this.state.email,
                                    "dateOfBirth": this.state.dob ? this.state.dob.format('yyyy-MM-dd') : null,
                                    "gender": this.state.valueGender,
    ...
    //         profileProvider.getInfoProfile(mobileNumber, fullName).then(s => {
    //             
    //             // switch (s.code) {
    //             //     case 0:
    //             //         NavigationService.navigate('verifyPhone', {
                ...
                <KeyboardAwareScrollView keyboardShouldPersistTaps='handled'>
                    <View style={styles.container}>
                        {this.state.badInfo ? <View style={styles.groupTitle}>
                            ...
                            <View></View>
                            // <View style={styles.groupTitle}> // tat ca comment khong duoc xoa, nghiep vu ngu nao lai sua lai :))
                        ...
                        {/* <View style={styles.containerScan}>
                                ...
                                <Text style={styles.txtOr}>Để điền thông tin nhanh hơn!</Text> // tat ca comment khong duoc xoa, nghiep vu ngu nao lai sua lai :))
                            </View> */}
                        <View>
                            <Text style={styles.txScanCode}>
                                Quét Mã iSofHcare của thành viên để điền thông tin tự động
                                </Text>
                            <TouchableOpacity onPress={this.onScanQrCode} style={styles.btnQrCode}>
                                <ScaledImage source={require('@images/new/profile/ic_qr.png')} height={23}></ScaledImage>
                        ...
                        <Form ref={ref => (this.form = ref)}>
                            ...
                            <Field style={styles.viewPhoneName}>
                                ...
                                <View style={styles.viewBtnCheck}>
                                    {
                                        this.state.isFinding ? <ActivityIndicator color={'#fff'}></ActivityIndicator> : <TouchableOpacity onPress={this.onFinding} style={styles.btnCheck}>
                                            <ScaledImage style={styles.imgCheck} source={require('@images/new/profile/ic_check_info.png')} height={23}></ScaledImage>
                            ...
                            {this.state.userDoesNotExist == 2 ? <Text style={styles.txResult}>Tìm thấy thông tin thành viên</Text> :
                                this.state.userDoesNotExist == 1 ?
                                    <Text style={styles.txResult}>
                                        Thành viên không tồn tại, vui lòng nhập thông tin!
                                </Text> : <Text style={styles.txResult}></Text>}

                            {this.state.findFinish ? <Field>
                                <Field style={[styles.containerField, {
                                    borderTopColor: '#00000011',
                                    borderTopWidth: 1
                                }]}>
                                ...
                                <Field style={[styles.containerField]}>
                                    ...
                                    <Field style={{ flex: 1 }}>
                                        <TextField
                                            ...
                                            splitDate={"/"}
                                            onPress={() => {
                                                this.setState({ toggleDateTimePickerVisible: true }, () => {
                                                })
                                            }}
                                            validate={{
                                                rules: {
