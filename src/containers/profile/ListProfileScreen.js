...
class ListProfileScreen extends Component {
  ...
  renderRelation = type => {
    switch (type) {
      ...
      default:
        return <Text style={styles.txtRelationShip} />;
    }
  };
  renderItem = ({item, index}) => {
    return item.medicalRecords.statusConfirm == 'NEED_CONFIRM' ? (
      <TouchableOpacity>
        <Text style={styles.txtmedicalRecords}>
          Tài khoản {item.medicalRecords.name} có số điện thoại{' '}
          {item.medicalRecords.phone} muốn xác nhận mối quan hệ với bạn.
        </Text>
        {item.medicalRecords.status == 1 ? (
          <TouchableOpacity
            ...
            <LinearGradient
              ...
              <Text style={styles.txProfileUser}>
              ...
              <View style={styles.viewProfileFamily}>
                <View>
                  ...
                  {item.medicalRecords.relationshipType ? (
                    <Text style={styles.txtRelationshipType}>
                      Quan hệ:{' '}
                      {this.renderRelation(
                        item.medicalRecords.relationshipType,
                      )}
                ...
                <View style={{flexDirection: 'row', alignItems: 'center'}}>
                  {item.medicalRecords.statusConfirm == 'NEED_CONFIRM' ? (
                    <TouchableOpacity
                      ...
                      onPress={() =>
                        this.onConfirm(
                          item.medicalRecords.id,
                          item.medicalRecords.sharePermission,
                          item.medicalRecords.medicalRelatedId,
                        )
                      }
                      style={{
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() =>
                      this.onShowOptions(
                        item.medicalRecords.id,
                        item.medicalRecords.permission,
                        item.medicalRecords.medicalRelatedId
                          ? item.medicalRecords.medicalRelatedId
                          : null,
                      )
                    ...
                    <ScaledImage
                      height={20}
                      width={20}
                      source={require('@images/new/profile/ic_three_dot.png')}
                    />
                  </TouchableOpacity>
                </View>
              </View>
          ...
          <View style={styles.viewProfileFamily}>
            <View>
              ...
              {item.medicalRecords.relationshipType ? (
                <Text style={styles.txtRelationshipType}>
                  Quan hệ:{' '}
                  {this.renderRelation(item.medicalRecords.relationshipType)}
                </Text>
              ) : (
                <View />
              )}
        ...
        <Modal
          ...
          <View style={styles.viewPopup}>
            <Text style={styles.txNotifi}>
              {constants.msg.user.want_delete_member}
            </Text>
            <View style={styles.viewBtn}>
              <TouchableOpacity
                onPress={this.onClickDone}
                style={styles.btnDone}>
                <Text style={styles.txDone}>
                  {constants.actionSheet.accept}
                </Text>
              </TouchableOpacity>
              <TouchableOpacity
