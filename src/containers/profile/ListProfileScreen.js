...
class ListProfileScreen extends Component {
        ...
        profileProvider.getListProfile().then(s => {
            ...
            this.setState({
                ...
                if (s.length) {
                    ...
                    //             let itemOwer = itemActive && itemActive.data.find(item => item.defaultProfile)
                    //             this.setState({
                    //                 data: [...newData],
                    //                 itemOwer
                    //             });
    ...
    renderTextRelations = (type) => {
        ...
        switch (type) {
            ...
            case "BOTHER": return 'ANH'
            case "YOUNG_BOTHER": return 'EM TRAI'
            case "SON": return 'CON'
    ...
    renderItem = ({ item, index }) => {
        let age = item?.profileInfo?.personal?.dateOfBirth ? new Date().getFullYear() - item?.profileInfo?.personal?.dateOfBirth?.toDateObject('-').getFullYear() : null
        return (
                ...
                <TouchableOpacity onPress={this.onClickItem(item)} style={styles.viewProfileUser}>
                    <View style={styles.viewActive}>
                        ...
                        <View style={styles.viewItemActive}>
                            <Text style={styles.nameActive}>{item?.profileInfo?.personal?.fullName}{item.defaultProfile ? ' (Chủ tài khoản)' : this.renderTextRelations(item.profileInfo?.personal.relationshipType)}
                            </Text>

                            {item?.profileInfo?.personal?.mobileNumber && <Text style={styles.phoneActive}>SĐT {item?.profileInfo?.personal?.mobileNumber}</Text>}
                            <Text style={styles.dobActive}>{item?.profileInfo?.personal?.value && item?.profileInfo?.personal?.hospitalName ? item?.profileInfo?.personal?.hospitalName : ''}</Text>
    ...
    createProfile = () => {
        ...
        // let itemOwer = this.state.data.find(obj => obj.defaultProfile)
        // itemOwer = itemOwer.profileInfo?.personal
        // let age = itemOwer.dateOfBirth ? new Date().getFullYear() - itemOwer.dateOfBirth.toDateObject('-').getFullYear() : null

        // if (itemOwer && (age || age == 0) && (itemOwer.gender) && itemOwer.address && itemOwer.fullName && itemOwer.phone) {
        //     if (age < 14 && itemOwer.guardian?.idNumber || age >= 14 && itemOwer.idNumber) {
        //         NavigationService.navigate('createProfile', {
        //             onCreate: this.onCreate.bind(this)
        //         })
        //     } else {
