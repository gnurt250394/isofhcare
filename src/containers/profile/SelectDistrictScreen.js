...
class SelectDistrictScreen extends Component {
    ...
    onRefresh = () => {
        ...
        this.setState({ refreshing: true }, () => {
            locationProvider.districtGetByProvince(id).then(s => {
                this.setState({
                    refreshing: false
                }, () => {
                    if (s) {
    ...
    render() {
        return (
            <ActivityPanel
                ...
                titleStyle={{ marginRight: 0 }} title={'Chọn Quận/Huyện'}
                isLoading={this.state.isLoading} menuButton={this.renderSearchButton()} showFullScreen={true}
            >
                {
                    this.state.showSearch ?
                        <View style={styles.containerSearch}>
                            <TextInput autoFocus={true} style={styles.inputSearch} placeholderTextColor='#dddddd' underlineColorAndroid="transparent" placeholder={constants.ehealth.inputKeyword} onChangeText={(s) => {
