...
class SearchIcdScreen extends Component {
  ...
  search = async () => {
    try {
      ...
      let res = await diseaseProvider.search(
    ...
    return (
      <View>
        <TouchableOpacity
          ...
          style={styles.btn}
          onPress={() => this.onShowDetails(index)}>
          <View style={styles.viewCode}>
            <Text style={styles.txCode}>{item.diseaseCode}</Text>
          </View>
          <View style={styles.viewName}>
            ...
            {item.children && item.children.length ? (
              <ScaledImage
                height={10}
                source={require('@images/new/icd/ic_right.png')}
              />
            ) : (
              <View />
            )}
