  ...
  componentDidMount() {
    ...
    userProvider.getAccountStorage(s => {
      setTimeout(() => {
        let decodedPayload = '';
        if (s?.loginToken) {
          const jwtParts = s.loginToken.split('.');
          const payloadInBase64UrlFormat = jwtParts[1];
          decodedPayload = base64Url.decode(payloadInBase64UrlFormat);
        }
        if (s && decodedPayload.indexOf('roleBase') != -1) {
            ...
            StackActions.reset({
              ...
              actions: [
                NavigationActions.navigate({
                  routeName: s && JSON.stringify(s) != '{}' ? 'home' : 'intro',
                }),
              ],
            }),
          );
      ...
      <ActivityPanel
        ...
        <View style={styles.container}>
          ...
          <View style={styles.containerCopyRight}>
            <ScaleImage
              source={require('@images/new/copyright.png')}
              height={20}
              style={{}}
            />
          </View>
