...
module.exports = {
  ...
  createBooking(date, description, hospitals, items, patientUser, time, idUser, images) {
    return new Promise((resolve, reject) => {
      let hospital = { id: hospitals && hospitals.id || '', name: hospitals && hospitals.name || '', address: hospitals && hospitals.address || hospitals.contact && hospitals.contact.address || '', placeCheckin: hospitals && hospitals.placeCheckin || '' }
